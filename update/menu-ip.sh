...
sleep 1
REQIP=$(curl -sS https://raw.githubusercontent.com/${USERGIT}/izinip/main/ip | awk '{print $4}' | grep $daftar)
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/remkenceng/izinip/main/ipx | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
